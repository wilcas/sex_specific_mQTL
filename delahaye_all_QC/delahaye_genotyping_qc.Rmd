...
```{r}
metadata <- read.delim("placenta_regulatory_landscape/RootStudyConsentSet_phs001717.PlacentalRegulation.v1.p1.c1.HMB-IRB-PUB-COL-MDS/PhenotypeFiles/Placental_Regulation_Phenotypes_clean.txt",sep ="\t",skip = 1)
...
fhet_data %>%
    ...
    ggplot(aes(F,fill=Self.declared.race)) + geom_histogram(bins=30)
```
...
plot_pcs <- function(dat,pc_col1,pc_col2,pc,what){
    p1 <- dat %>%
      inner_join(metadata,by = c("V2"="SUBJECT_ID")) %>%
      mutate(Self.declared.race= as.factor(Self.declared.race)) %>%
      ggplot(aes_string(pc_col1,pc_col2,color="Self.declared.race")) +
        geom_point() +
        labs(x=sprintf("PC%d",pc),y=sprintf("PC%d",pc+1)) +
        ggtitle("PCA Placental Regulation")
    p2 <- dat %>%
      inner_join(metadata,by = c("V2"="SUBJECT_ID")) %>%
      mutate(Self.declared.race= as.factor(Self.declared.race)) %>%
      ggplot(aes_string(pc_col1,"Gestational.Age.at.Delivery",color="Self.declared.race")) +
        geom_point() +
...
library(qqman)
library(data.table)
reg_results <- fread("TERRE_QC/imputed_dosage_batch_1.PD1.glm.logistic.hybrid")
qq(reg_results$P)
