...
delahaye_meta <- delahaye_meta %>% right_join(delahaye_ancestry,by=c("IID"="V2")) %>% right_join(delahaye_meta_methy,by="SUBJECT_ID")
...
for(i in 0:20){
    if (i==1){
        ...
        tmp <- t(cbind(methy_PC$x[,1:i],batch=batch_matrix[,-c(1)],
...
ggplot(to_plot,aes(PC,hits)) + geom_point()+geom_smooth()
```
