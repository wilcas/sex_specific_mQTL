...
setwd("/scratch/st-dennisjk-1/wcasazza/1000G_phase3_ldsc/baseline_EUR_annot_files/")
...
out <- mclapply(
  ...
  function(i) {
    ...
    colnames(dt) <- names
    dt$base <- 1
    bim <- fread(sprintf("/project/st-dennisjk-1/shared/data/1000G_EUR_ldsc_data/1000G_EUR_Phase3_plink/1000G.EUR.QC.%d.bim", i), header = F)
