...
if (file.exists(sprintf("%sdelahaye_caviar_cpp_annotations_chr%s.annot.gz", argv[[2]], argv[[1]]))) {
  ...
  tmp <- fread(sprintf("/arc/project/st-dennisjk-1/shared/data/1000G_phase3_by_chr/plink/mapped_maf01_chr%s.bim", argv[[1]]))
  ...
  fwrite(merged, sprintf("%s.%s.annot.gz", argv[[3]], argv[[1]]), row.names = F, quote = F, sep = "\t")
} else {
