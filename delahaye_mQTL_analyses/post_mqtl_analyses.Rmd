...
```
```{r marginal hits}
opt_pc <- vroom("matrix_eqtl_data/cis_mQTL_9_methy_PC_all.txt.gz", altrep = TRUE)
delahaye <- vroom("delahaye_2020_mQTL.csv")
# hist(opt_pc$`p-value`, breaks = 100)
sig_delahaye <- opt_pc %>% filter(FDR < 0.05)
```
## Replication
Delahaye and itself:
```{r mqtl replication}
delahaye_repeat_merged <- opt_pc %>% right_join(delahaye, by.x = c("SNP", "gene"), by.y = c("SNPID", "cpgID"))
